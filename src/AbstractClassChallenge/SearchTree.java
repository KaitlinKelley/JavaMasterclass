...
//THIS IS AN ACTUAL NIGHTMARE WHAT IS GOING ON HERE
public class SearchTree implements NodeList{
    ...
    public boolean addItem(ListItem newItem) {
        if(this.root == null){
            //tree was empty, so new item becomes root
            this.root = newItem;
            return true;
        }
        ...
        //can't actually get here, but we need this return to make IntelliJ shut up
        return false;
    }
    ...
    private void performRemoval(ListItem item, ListItem parent){
        ...
        if(item.next() == null){
            ...
            //neither left nor right tree are null, so deletion is trickier
            //From the right sub-tree, find the smallest aka leftmost value
            ...
            if(leftmostParent == item){
                //there was no leftmost node, so "current" points to the smallest node(the one that now must be deleted)
